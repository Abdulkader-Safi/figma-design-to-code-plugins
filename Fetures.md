...
- **Semantic tags from node type and layer name.** Use `button`, `nav`, `header`, `section`, `ul`, `h1` to `h6`, `p`, and `img` where the intent is clear, instead of `div` everywhere. Big readability jump for a small change.
...
- **Tailwind CSS output mode.** This is the single most requested format across every tool. Map spacing, colors, flex, and typography to Tailwind utility classes. Offer arbitrary values for anything off-scale.
